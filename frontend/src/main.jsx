        ...
        <div className="notification-bell-container">
          ...
          {showNotifications && (
            <div className="panel notification-dropdown">
              <h3 style={{ fontSize: "16px", fontWeight: "700", borderBottom: "1px solid var(--line)", paddingBottom: "8px", marginBottom: "8px" }}>
                Notifications & Updates
              </h3>
              <div className="note-list">
                {notifications.length > 0 ? (
                  notifications.map((note) => (
                    <div key={note._id} className="note-item" style={{ flexDirection: "column", alignItems: "flex-start", gap: "4px", padding: "10px" }}>
                      <div className="note-content">
                        <strong style={{ fontSize: "13.5px" }}>{note.title}</strong>
                        <p style={{ fontSize: "12.5px", color: "var(--muted)", margin: 0 }}>{note.message}</p>
                      </div>
                      <span className="note-time" style={{ fontSize: "10px", alignSelf: "flex-end" }}>{new Date(note.createdAt).toLocaleDateString()}</span>
                    </div>
                  ))
                ) : (
                  <div className="empty" style={{ padding: "16px", fontSize: "13px" }}>No notifications yet.</div>
                )}
              </div>
            </div>
          )}
...
function Profile({ user, request, refresh, triggerSuccess, triggerError }) {
      ...
      {loading ? (
        ...
        roadmap && (
          <section style={{ display: "flex", flexDirection: "column", gap: "24px", width: "100%" }}>
            {/* Job Details Section at the top, spanning full width */}
            <div className="panel" style={{ width: "100%" }}>
              <h2>Job Details</h2>
              <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(220px, 1fr))", gap: "16px", marginTop: "16px" }}>
                <p><strong>Required Experience:</strong> <span style={{ textTransform: "capitalize" }}>{roadmap.job.experienceLevel}</span></p>
              ...
              <div style={{ borderTop: "1px solid var(--line)", paddingTop: "12px", marginTop: "12px" }}>
                ...
                <p style={{ color: "var(--muted)", marginTop: "6px", fontSize: "14.5px", lineHeight: "1.6" }}>{roadmap.job.description}</p>
              </div>
            </div>

            {/* Roadmap section at the bottom, spanning full width */}
            <div className="panel" style={{ width: "100%" }}>
                              ...
                              gap.resources.map((res) => (
                                <div key={res._id} style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                                  ...
                                  <div style={{ display: "flex", gap: "8px", alignItems: "center" }}>
                                    ...
                                    <a href={res.w3schoolsUrl || "https://www.w3schools.com"} target="_blank" rel="noopener noreferrer">
                                      <button className="secondary" style={{ minHeight: "30px", padding: "0 10px", fontSize: "12px", borderColor: "var(--primary)", color: "var(--primary)" }}>
                                        W3Schools
                                      </button>
                                    </a>
                                  </div>
...
function ResourcesManager({ request, triggerSuccess, triggerError }) {
  ...
  async function handleAdd(event) {
    ...
    const values = {
      ...
      url: fd.get("url"),
      w3schoolsUrl: fd.get("w3schoolsUrl") || "https://www.w3schools.com",
      type: fd.get("type"),
      skillsTaught
    };
    ...
    <>
      ...
      <section className="grid">
        <form className="panel form span-4" onSubmit={handleAdd} style={{ alignSelf: "flex-start" }}>
          ...
          <label>W3Schools Link <input name="w3schoolsUrl" placeholder="https://www.w3schools.com" defaultValue="https://www.w3schools.com" /></label>
          <label>
            Type
            <select name="type">
                    ...
                    <tr key={res._id}>
                      <td>
                        ...
                        <div style={{ fontSize: "12px", color: "var(--muted)" }}>
                          {res.provider} • <a href={res.url} target="_blank" rel="noopener noreferrer" style={{ color: "var(--accent)" }}>Visit Link</a> • <a href={res.w3schoolsUrl || "https://www.w3schools.com"} target="_blank" rel="noopener noreferrer" style={{ color: "var(--primary)", fontWeight: "600" }}>W3Schools Link</a>
                        </div>
                      </td>
